...
## [为什么需要链表](https://labuladong.online/algo/data-structure-basic/linkedlist-basic/#%E4%B8%BA%E4%BB%80%E4%B9%88%E9%9C%80%E8%A6%81%E9%93%BE%E8%A1%A8)
...
> [!NOTE|aside-r] 链表的内存特性：不连续
> 优点：内存利用率高，节点之间的连接、拆分都更灵活，无需考虑扩容、缩容；
> 缺点：不支持按索引访问，查找效率低；
...
## [单链表的基本操作](https://labuladong.online/algo/data-structure-basic/linkedlist-basic/#%E5%8D%95%E9%93%BE%E8%A1%A8%E7%9A%84%E5%9F%BA%E6%9C%AC%E6%93%8D%E4%BD%9C)
...
### [查/改](https://labuladong.online/algo/data-structure-basic/linkedlist-basic/#%E6%9F%A5-%E6%94%B9)
...
### [增](https://labuladong.online/algo/data-structure-basic/linkedlist-basic/#%E5%A2%9E)
...
### [删](https://labuladong.online/algo/data-structure-basic/linkedlist-basic/#%E5%88%A0)
...
在单链表尾部删除元素
> [!NOTE|aside-r] 单链表尾部删除元素：通过 while(current. Next. Next! = null) 循环，找到倒数第二个节点
...
// 现在链表变成了 2 -> 3 -> 4 -> 5
```
> [!NOTE|aside-r] 链表节点删除时，被删除节点 A不会造成内存泄漏
> 此时已经没有别的节点的 next 指针指向 A节点了，因此可以被垃圾回收掉
...
虚拟节点技巧在 [单链表经典算法技巧](https://labuladong.online/algo/essential-technique/linked-list-skills-summary/) 中也会经常运用，这里仅仅简单提一下，具体实现会在后面讲到。

## [双链表的基本操作](https://labuladong.online/algo/data-structure-basic/linkedlist-basic/#%E5%8F%8C%E9%93%BE%E8%A1%A8%E7%9A%84%E5%9F%BA%E6%9C%AC%E6%93%8D%E4%BD%9C)

我先写一个工具函数，用于创建一条双链表，方便后面的讲解：

java
> [!NOTE|aside-r] 数组转化为双链表
> 小优化：是否可以将头节点的前驱指针 pre，置为 null？
```
...
### [查/改](https://labuladong.online/algo/data-structure-basic/linkedlist-basic/#%E6%9F%A5-%E6%94%B9-1)
...
```
> [!NOTE|aside-r] 双链表向前、向后遍历双链表的选择

==访问或修改节点时，可以根据索引是靠近头部还是尾部，选择合适的方向遍历，这样可以一定程度上提高效率。==

### [增](https://labuladong.online/algo/data-structure-basic/linkedlist-basic/#%E5%A2%9E-1)

在双链表头部插入新元素

在双链表头部插入元素，需要调整新节点和原头节点的指针：
> [!NOTE|aside-r] 注意别漏掉指明链表的头、尾节点!!!
> 每次新增或删除节点时，一定不要忘记给 head/tail 引用赋值，指明当前链表的起点或终点
...
// 现在链表变成了 0 -> 1 -> 2 -> 3 -> 4 -> 5
```
> [!NOTE|aside-r] 双链表的尾指针 tail 大概率是空的
> 可能还是要依赖头指针 head，来操作双链表
...
在双链表中间插入新元素
> [!NOTE|aside-r] 链表插入新元素的两步重要操作（可防止漏操作）
> 1. 先组装新节点；
> 2. 再修改原链表的结构（调整前驱、后继节点的指针），此时才是真正意义上的插入动作；
...
### [删](https://labuladong.online/algo/data-structure-basic/linkedlist-basic/#%E5%88%A0-1)
...
## [接下来](https://labuladong.online/algo/data-structure-basic/linkedlist-basic/#%E6%8E%A5%E4%B8%8B%E6%9D%A5)
